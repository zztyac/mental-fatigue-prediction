"""
Metal Fatigue Life Prediction Model

This model combines CNN network for processing time series data and fully connected neural network 
...
class CNNModel(nn.Module):
    """
    CNN model for processing time series data
    """
    def __init__(self, input_dim):
        super().__init__()
        self.nor = nn.BatchNorm1d(input_dim)   # Batch normalization layer
...
class CombinedModel(nn.Module):
    """
    Hybrid model combining DNN and CNN
    """
    def __init__(self, input_dim):
        super().__init__()
        self.strct_block = DNNModel(input_dim)  # Structural feature processing module
        self.cnn_block = CNNModel(input_dim)  # Time series data processing module
    ...
    def forward(self, b_x, b_x_csv):
        ...
        x_cnn = self.cnn_block(b_x_csv)      # Process time series data
...
def plot_loss_curves(train_loss, test_loss, save_path="/home/zty/code/mental_fatigue/cnn/lossvalue"):
    """
    Plot loss curves
...
def main():
    ...
    cnn_block = CNNModel(dim_num).to(device)
    combined_model = CombinedModel(dim_num).to(device)
    
    # Print model structure
    print_model_structure(combined_model)
    ...
    # Train model
    epochs = 100
    model_save_path = '/home/zty/code/mental_fatigue/cnn/best_model_weights_CNN.pth'
    ...
    save_predictions_to_csv(predictions, labels, "/home/zty/code/mental_fatigue/cnn/predictions_and_labels_train-CNN.csv")
